...
#
# Author: Ted Bohn
# $Id: agg_time.pl,v 1.1 2004/06/18 00:56:29 tbohn Exp $
#-------------------------------------------------------------------------------
...
@supported_input_intervals = ("hourly", "daily", "monthly");
@supported_output_intervals = ("daily", "monthly", "yearly", "annual");
...
$status = &GetOptions(\%options_hash, "h", "i=s","o=s","in=s","out=s","avg=s@","sum=s@", "beg=s@");
...
if ($in_interval ne "hourly" && $out_interval eq "daily") {
...
foreach (<INFILE>) {
  ...
  # Compare current year/month/day to previous record's year/month/day.
  # If this is the beginning of the next aggregation interval,
  # do end-of-interval stuff
  if ( ($out_interval eq "daily" && $day != $prev_day)
    ...
    for ($i=0; $i<=$#fields-$first_data_field; $i++) {
      if ($agg_method{$i} eq "sum" || $agg_method{$i} eq "beg") {
...
sub usage() {
  ...
  print "\n";
  print "usage:\n";
  print "  perl $0 [-h] -i <infile> -o <outfile> [-in <in_interval>] [-out <out_interval>]\n";
  ...
  if ($_[0] eq "full") {
    ...
    print "      hourly  = input time step is sub-daily (any length less than 24\n";
    ...
    print "      perl $0 -i in_file -o out_file -sum 5\n";
    print "\n";
    ...
    print "      perl $0 -i in_file -o out_file -in hourly -sum 5,6\n";
    print "\n";
    print "    To aggregate a file of 3-hourly records to yearly values, where fields\n";
    print "    5 and 6 are to be summed instead of averaged:\n";
    print "      perl $0 -i in_file -o out_file -in hourly -out yearly -sum 5,6\n";
    print "\n";
  }
